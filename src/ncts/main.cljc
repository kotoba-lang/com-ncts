(ns ncts.main
  "Kotodama WASM entrypoint for the Ncts clean-room actor (L4) — Clojure port.
  ...
  facts (`ncts.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"ncts.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ncts-compat" :tier tier :entities entities} 200])
